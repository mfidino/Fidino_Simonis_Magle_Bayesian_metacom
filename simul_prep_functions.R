...
#
#
# simul_prep_functions for simulation work
#
# 1/23/2016
#
# written by Mason Fidino

# layout of all the functions.
...
gen_sim_list <- function(nsite = NULL, nspec = NULL, nyear = NULL,
  ...
  # cutoff, for another type of simulation. If 
  # there is a value then you remove all data for any sampling
  # that has fewer than cut_off observations.
...
# this makes the observed data set
...
# the simulated values. Not currently used
...
# write the mcmc_matrix. Not currently used
...
# append that into a table. Not currently used.
...
# from the sim_list. Not the z matrix though. Not currently used
...
# again, used within a loop. Not currently used.
...
batch_analyze <- function(all_sim = NULL, params = NULL,
                          ...
                          model = NULL, comparison_model = NULL,
                          m_name = NULL, save_path = NULL){
  ...
  for(i in 1:length(all_sim)){
    ...
    mod_mcmc_path <- paste(save_path, "/post_matrix_", m_name,i, ".txt", sep = "")
    write.table(as.matrix(mod_mcmc, chains = TRUE), mod_mcmc_path,
                row.names = FALSE, sep = "\t")
    saveRDS(all_sim[[i]], paste(save_path,"/known_",m_name, i, ".txt", sep = ""))
    ...
    if(make_comparisons){
      ...
      mod_mcmc_compare <- as.mcmc.list(run.jags( model=comparison_model , 
                                                 monitor=params , 
                                                 data=all_sim[[i]]$data_list,  
                                                 inits=inits_beta , 
                                                 n.chains=n_chains ,
      ...
      mod_mcmc_path <- paste(save_path, "/post_comp_matrix_",m_name,i, ".txt", 
                             sep = "")
...
###----------------------------------------------------------------------------
#     package_load    package_load    package_load    package_load       
###----------------------------------------------------------------------------

package_load<-function(packages = NULL, quiet=TRUE, 
                       verbose=FALSE, warn.conflicts=FALSE){
  
  # download required packages if they're not already downloaded.
  # Then load them
